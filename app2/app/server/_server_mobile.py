	...
	if request.method=='POST':
		...
		if usr_pwd['username'] and usr_pwd['password']:
			if sha256_crypt.verify(password_candidate, usr_pwd['password']):
				...
				print('HIII ',session['username'],' ',usr_pwd['password'])
				#print(request.form)
...
def add_review():
	...
	## Apps form is not passing the validate command for some reason.
	#if request.method == 'POST' and form.validate():
	if request.method == 'POST':
		print('HIHIHIHHHHHHHH')
		area = form.area.data
		...
		rating = int(float(form.rating.data))
		comments = form.comments.data

		print(request.form)

		assert len(lat_split)==len(long_split),"Enter Same number of lats and longs"
		coords_list = [[float(long_split[i]),float(lat_split[i])] for i in range(len(lat_split))]
	...
	return jsonify({'message':'addreview '})
