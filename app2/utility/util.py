...
def user_view_reviews(link, db_name, collection_name, user_id):
	...
	return jsonify(db.find_one(db_name, collection_name, query_find_user_data, project_data))
...
def view_overall_reviews(link, db_name, collection_name, scale, center_of_screen_gps_coords, categories):
	...
	geo_near_query = {
        ...
        return jsonify(db.createIndex(db_name, collection_name, "ratings.coords_list", pymongo.GEO2D, geo_near_query))
        '''
...
##
def view_closest_point_stats(link, db_name, collection_name, scale, point_selected):
	#db = mdb.DaoImpl(link, db_name, collection_name)
        db = mdb.DaoImpl.getInstance()
...
##
def view_friends_reviews(link, db_name, collection_name, friend, friend_id):
	#db = mdb.DaoImpl(link, db_name, collection_name)
        db = mdb.DaoImpl.getInstance()
