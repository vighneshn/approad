...
class DaoImpl:
	...
	##
        ...
        ##
        def createIndex(self, db_name, collection_name, index_name, index_type, query):
                print('HELLOOOOO')
                self.get_collection(db_name, collection_name).ensure_index([(index_name, index_type)])
                print('HELLOOOOO!!!!!!!')
                return self.find_all(db_name, collection_name, query)
                '''cursor = self.__client[db_name][collection_name].find(query)
        	#total = cursor.count()
                print(cursor)
        	# data =list(cursor)
        	data=[]
